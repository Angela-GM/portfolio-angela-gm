...
const recoveryUser: RecoverUserDto = {
  ...
  password: '123456',
  recovery_token:
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2NDkwYzlkM2U3N2RhNzNiM2ZkMDYyOTkiLCJlbWFpbCI6Impob25AY29ubm9ycy5jb20iLCJpYXQiOjE2ODczODg3NTcsImV4cCI6MTY4NzM4ODc2N30.FQjOTQSBrZSVJ1AhJ5EBpAsx_XMaXY39sTvvFSI7uOs',
};
...
describe('UsersService', () => {
  ...
  const mockUsersService = {
    updatePassword: jest
      .fn()
      .mockImplementation((recoveryUser: RecoverUserDto) => {
        return {
  ...
  it('deleteUserByAdmin() should return standard response if a user is deleted', async () => {
    expect(
      await service.deleteUserByAdmin(
        new mongoose.Schema.Types.ObjectId('6490c8971191c2854f5453f2'),
      ),
    ).toMatchObject({
      message: 'User deleted by Admin',
      status: HttpStatus.OK,
